...
$defaultBreadcrumbs = [
    ...
    'Crawler' => backpack_url('plugin/chilltv-crawler'),
];
                ...
                <div class="card-body">
                    <div class="row">
                        ...
                        <div class="form-group col-12 mb-3">
                            ...
                            <textarea class="form-control" rows="5" name="link">https://chilltv.xyz/danh-sach/phim-moi-cap-nhat</textarea>
    ...
    <script>
        ...
        $('.btn-load').click(function(e) {
            ...
            const fetchApi = async (link, from, to) => {
                isFetching = true;
                const response = await fetch("{{ backpack_url('plugin/chilltv-crawler/fetch') }}?" +
                    new URLSearchParams({
                        link,
                        from,
                        to
        ...
        const processMovie = async (slug) => {
            ...
            const response = await fetch("{{ backpack_url('plugin/chilltv-crawler/crawl') }}", {
                method: 'POST',
                headers: {
        ...
        $("#movies-get-handle").click(async function() {
            const apiDomain = "{{ config('chilltv_crawler.domain', 'https://chilltv.xyz') }}";
            let params = $("select[name=movies-get-params]").find(":selected").val();

            const response = await fetch("{{ backpack_url('plugin/chilltv-crawler/get-movies') }}", {
                method: 'POST',
                headers: {
                    "Content-Type": "application/json",
                    'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
